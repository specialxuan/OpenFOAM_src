...
- 每个时间步：`dynamicFvMesh` 基类在求解器时间循环中调用 `update()`；`fastDynamicFvMesh`::`update()` 按顺序执行：采样流体界面力、投影到模态、解结构模态方程、松弛应用（`couplingRelaxation`），重建节点位移并调用 `movePoints()` 更新网格、写诊断（`modal_diagnostics.csv`、可选面级文件）。
  注意：`update()` 函数包含防止单时间步内多次重复更新结构状态的机制。若求解器（如 PIMPLE 循环）在同一时间步多次调用网格更新，本程序仅在第一次调用时推进结构动力学方程，后续调用仅更新网格位置而不改变结构状态，从而避免错误的加速度累积。
...
- `pRef`: 参考压力（默认 `0`）。用于从流场压力 `p` 中减去该值以计算流体载荷，适用于绝对压力求解器需转换为相对压力计算力的情况。
